    ...
    const measurePageLoad = () => {
      if ('performance' in window && 'getEntriesByType' in performance) {
        ...
        if (navigation) {
          ...
          logMetric({ name: 'DOMContentLoaded', value: domContentLoaded, timestamp: Date.now() });
          
          // Page Load Complete
          const loadComplete = navigation.loadEventEnd - navigation.fetchStart;
          logMetric({ name: 'LoadComplete', value: loadComplete, timestamp: Date.now() });
        }
      }
    };
...
export function useRenderTime(componentName: string) {
  useEffect(() => {
    ...
    return () => {
      ...
      if (renderTime > 16) { // Log renders that take longer than one frame (16ms)
        logMetric({
